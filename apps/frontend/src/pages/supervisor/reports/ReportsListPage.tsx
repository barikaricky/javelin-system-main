...
export default function SupervisorReportsListPage() {
  ...
  const fetchReports = async (isRefresh = false) => {
    try {
      ...
      console.log('📊 Fetching reports for Supervisor...');
      const response = await api.get('/reports');
      console.log('✅ Reports fetched:', response.data);
      
      // CRITICAL: Filter to show ONLY reports created by current supervisor
      const allReports = response.data.reports || [];
      const myReports = allReports.filter((report: Report) => 
        report.createdBy && report.createdBy._id === (user as any)?._id
      );
      
      console.log(`🔍 Filtered ${myReports.length} own reports out of ${allReports.length} total`);
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                View and manage your security reports
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Link
                to="/supervisor/reports/analytics"
          ...
          <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Search by title, BEAT, or location..."
                  value={searchTerm}
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Edit button ONLY for DRAFT or REVISION_REQUIRED */}
                        {(report.status === 'DRAFT' || report.status === 'REVISION_REQUIRED') && (
                          <Link
                            to={`/supervisor/reports/${report._id}/edit`}
                            className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all"
                            title="Edit Report"
                          >
                        ...
                        {/* NO DELETE BUTTON for Supervisor */}
                      </div>
                    </div>
                  </div>
                </div>
