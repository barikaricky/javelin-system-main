...
import { Users, Search, Shield, MapPin, Phone, Mail, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export default function SupervisorsListPage() {
  ...
  const fetchSupervisors = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch supervisors');
      setSupervisors([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredSupervisors = supervisors.filter((supervisor) => {
    const matchesSearch = 
      ...
      supervisor.users?.email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      supervisor.employeeId?.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesStatus = statusFilter === 'all' || supervisor.users?.status === statusFilter;
    
    const matchesType = typeFilter === 'all' || supervisor.supervisorType === typeFilter;
  ...
  const stats = {
    ...
    active: supervisors.filter(s => s.users?.status === 'ACTIVE').length,
    pending: supervisors.filter(s => s.users?.status === 'PENDING').length,
    supervisors: supervisors.filter(s => s.supervisorType === 'SUPERVISOR').length,
    generalSupervisors: supervisors.filter(s => s.supervisorType === 'GENERAL_SUPERVISOR').length,
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">View all supervisors and their information</p>
        </div>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-indigo-600">{stats.generalSupervisors}</p>
              </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {supervisor.users?.profilePhoto ? (
                      <img
                        src={supervisor.users.profilePhoto}
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">{supervisor.fullName}</h3>
                      <p className="text-sm text-gray-500">{supervisor.employeeId}</p>
                    </div>
                  </div>
                  {supervisor.users?.status && getStatusBadge(supervisor.users.status)}
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  ...
                  {supervisor._count && (
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-900">{supervisor._count.operators || 0}</p>
                    </div>
                  )}
                  {supervisor.regionAssigned && (
                    <div>
                      <p className="text-xs text-gray-500">Region</p>
                      <p className="text-sm font-medium text-gray-900">{supervisor.regionAssigned}</p>
                    </div>
                  )}
                </div>
              </div>
