...
import { MapPin, Building2, Phone, User, FileText, Save, ArrowLeft, Loader2 } from 'lucide-react';
...
export const EditLocationPage = () => {
  ...
  useEffect(() => {
    const fetchLocation = async () => {
      ...
      try {
        ...
        // Update LGAs based on the state
        if (location.state && STATE_LGAS[location.state]) {
          setAvailableLGAs(STATE_LGAS[location.state]);
        }
  ...
  const handleCitySelect = (city: { name: string; state: string }) => {
    ...
    setErrors(prev => ({ ...prev, city: '', state: '', lga: '' }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/api/locations/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      console.log('Location updated:', response.data);
      toast.success('Location updated successfully!');
      navigate('/secretary/locations');
    } catch (error: any) {
      console.error('Error updating location:', error);
      const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to update location';
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              The location you're trying to edit doesn't exist or may have been deleted.
            </p>
            <button
              onClick={() => navigate('/secretary/locations')}
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Update location information for your organization</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Type or select from common suggestions</p>
          </div>
          ...
          <div>
            ...
            {availableLGAs.length > 0 ? (
              ...
              <input
                ...
                value={formData.lga}
                onChange={handleChange}
                placeholder={formData.state ? "Enter LGA manually" : "Select a state first"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {!errors.contactPhone && formData.contactPhone && (
                <p className="mt-1 text-sm text-gray-500">Nigerian phone format accepted</p>
              )}
            </div>
          </div>
