...
export default function BitExpensesPage() {
  ...
  const loadBits = async () => {
    try {
      // Get all bits (both active and inactive)
      const response = await api.get('/bits', { params: { isActive: 'all' } });
      const loadedBits = response.data.bits || [];
      
      setBits(loadedBits);
      setFilteredBits(loadedBits);
      
      if (loadedBits.length === 0) {
        console.warn('No BITs found in database.');
      }
    } catch (error: any) {
      console.error('Error loading BITs:', error);
      toast.error(error.response?.data?.message || 'Failed to load BITs');
    }
  };
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/bit-expenses', payload);
  ...
  const handleDeleteExpense = async (id: string) => {
    ...
    try {
      await api.delete(`/bit-expenses/${id}`);
  ...
  const openDetailModal = async (summary: BitSummary) => {
    setSelectedBitDetails(summary);
    setShowDetailModal(true);
    // Load expenses for this specific BIT
    try {
      const response = await api.get('/bit-expenses', {
        params: { bitId: summary.id }
      });
      setExpenses(response.data.expenses || []);
    } catch (error) {
      console.error('Error loading BIT expenses:', error);
      toast.error('Failed to load expenses');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
                ...
                <div key={summary.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={async () => {
                        if (confirm(`Are you sure you want to delete all expenses for "${summary.name}"? This will NOT delete the BIT itself, only its expenses. This action cannot be undone.`)) {
                          try {
                            const response = await api.delete(`/bit-expenses/bit/${summary.id}/all`);
                            toast.success(response.data.message || 'Expenses deleted successfully');
                            loadBitSummaries();
                            loadExpenses();
                          } catch (error: any) {
              ...
              <div className="p-6">
                ...
                <form onSubmit={showAddModal ? handleAddExpense : handleEditExpense} className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Select a location first, then optionally select a BIT</p>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      BIT {selectedLocationId ? '(filtered by location)' : '(select location first)'}
                    </label>
                    <select
        ...
        {/* Detail Modal - Show all expenses for a specific BIT/Location */}
        {showDetailModal && selectedBitDetails && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-sm font-semibold text-blue-600 mb-1">{selectedBitDetails.locationName}</p>
                    <h2 className="text-2xl font-bold text-gray-900">{selectedBitDetails.name}</h2>
                    <p className="text-sm text-gray-600">{selectedBitDetails.clientName}</p>
                  </div>
                  <button
                    onClick={() => {
                      setShowDetailModal(false);
                      setSelectedBitDetails(null);
                    }}
                    className="text-gray-400 hover:text-gray-600"
                  >
                    <X className="w-6 h-6" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-blue-600">{formatCurrency(selectedBitDetails.totalAmount)}</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-sm text-gray-600 mb-1">Number of Expenses</p>
                    <p className="text-2xl font-bold text-green-600">{selectedBitDetails.expenseCount}</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <p className="text-sm text-gray-600 mb-1">Average Expense</p>
                    <p className="text-2xl font-bold text-purple-600">{formatCurrency(selectedBitDetails.averageExpense)}</p>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                            ...
                            <button
                              onClick={async () => {
                                if (confirm('Are you sure you want to delete this expense?')) {
                                  ...
                                  openDetailModal(selectedBitDetails);
                                }
                              }}
                              className="text-red-600 hover:text-red-900"
                              title="Delete"
                  ...
                  {expenses.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      No expenses found for this BIT/Location
                    </div>
                  )}
                </div>
              </div>
